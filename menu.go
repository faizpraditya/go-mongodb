...
func CountStudent(ctx context.Context, coll *mongo.Collection) {
	total, err := coll.CountDocuments(ctx, coll)
...
func CountStudentByAge(ctx context.Context, coll *mongo.Collection, age int) {
	total, err := coll.CountDocuments(ctx, bson.M{"age": age})

	if err != nil {
		log.Fatalln(err)
	}
	fmt.Printf("Total documents by age %v : %v \n", age, total)
}
